...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not await _require_private_chat(update):
        return
    context.user_data[FLOW_KEY] = new_flow(mode="add")
    text, markup = step_prompt(context.user_data[FLOW_KEY])
...
async def cancel(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not await _require_private_chat(update):
        return
    cleared = False
    if context.user_data.pop(FLOW_KEY, None) is not None:
        cleared = True
...
async def edit(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await _reply(
        update,
        step_prompt(flow)[0],
        reply_markup=confirm_keyboard(str(flow.get("direction") or ""), mode="edit"),
    )
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if flow is not None:
        ...
        if step == "confirm":
            await _reply(update, "Use the buttons to Save/Change fields, or send /cancel.")
            return
...
async def add_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if action.startswith("confirm:edit:"):
        if flow.get("step") != "confirm":
            text, markup = step_prompt(flow)
            await _edit_or_reply_query_message(query, text, reply_markup=markup)
            return
        _, _, field = action.rpartition(":")
        if field == "type":
            flow["step"] = "choose_type"
        elif field == "person":
            flow["step"] = "person"
    ...
    if action == "confirm:save":
        ...
        if direction not in {"expense", "payable", "receivable"} or not isinstance(amount, int):
            await _edit_or_reply_query_message(query, "Missing required fields. Use /cancel.")
            return

        if direction in {"payable", "receivable"} and not (flow.get("person") or "").strip():
            flow["step"] = "person"
            await _edit_or_reply_query_message(query, "Counterparty is required.")
            text, markup = step_prompt(flow)
            await _edit_or_reply_query_message(query, text, reply_markup=markup)
            return
        ...
        try:
            if mode == "edit":
                ...
                if not isinstance(tx_id, int):
                    await _edit_or_reply_query_message(query, "Invalid record id for edit. Use /cancel.")
                    return
                ok = await update_transaction(parsed, user_id=uid, tx_id=int(tx_id), pool=_db_pool(context))
                if not ok:
                    await _edit_or_reply_query_message(query, "Not found (or not yours).")
                    return
                saved_id = int(tx_id)
            else:
            ...
            await _edit_or_reply_query_message(query, "Sorry, I couldn't save that right now. Try again.")
            return

        saved_text = format_saved(flow, tx_id=int(saved_id))
        clear_flow(context.user_data)
        await _edit_or_reply_query_message(query, saved_text)
        return
